...
jR.route("/users/", function () {
  if (user.isadmin) SET("common.page", "users");
  else jR.redirect("/");
});
...
function marked_video(selector) {
  selector.each(function () {
    ...
    if (html.indexOf("youtube") !== -1) {
      ...
      el.parent().replaceWith(
        '<div class="video"><iframe src="//player.vimeo.com/video/' +
...
Tangular.register("photo", function (value) {
  return value ? "/photos/{0}.jpg".format(value) : "/img/avatar.svg";
});
...
Tangular.register("labels", function (value) {
  ...
  value
    ...
    .forEach(function (label) {
      ...
      builder.push(
        '<span class="label label-{1}">{0}</span>'.format(
          Tangular.helpers.encode(label),
          index === -1 || index > 8 ? 8 : index
        )
      );
    });
  return builder.join("");
