COMPONENT("click", function () {
  ...
  self.click = function () {
    ...
    if (typeof value === "string") self.set(self.parser(value));
    else self.get(self.attr("data-jc-path"))(self);
  };
...
COMPONENT("visible", function () {
  ...
  self.setter = function (value) {
    ...
    if (condition) is = self.evaluate(condition);
    else is = value ? true : false;
...
COMPONENT("message", function () {
  ...
  self.content = function (cls, text, icon) {
    !is &&
      self.html(
        '<div><div class="ui-message-body"><span class="fa fa-warning"></span><div class="ui-center"></div></div><button>' +
          (self.attr("data-button") || "Close") +
          "</button></div>"
      );
    timer && clearTimeout(timer);
    visible = true;
    self
      .find(".ui-message-body")
      .removeClass()
      .addClass("ui-message-body " + cls);
    self
      .find(".fa")
      .removeClass()
      .addClass("fa " + icon);
...
COMPONENT("checkbox", function () {
  ...
  self.validate = function (value) {
    ...
    if (type === "undefined" || type === "object") value = "";
    else value = value.toString();

    return value === "true" || value === "on";
  };

  if (!required) self.noValid();

  self.make = function () {
    self.classes("ui-checkbox");
    self.html(
      '<div><i class="fa fa-check"></i></div><span{1}>{0}</span>'.format(
        self.html(),
        required ? ' class="ui-checkbox-label-required"' : ""
      )
    );
...
COMPONENT("dropdown", function () {
  ...
  self.validate = function (value) {
    if (select.prop("disabled") || !isRequired) return true;
  ...
  self.required = function (value) {
    self
      .find(".ui-dropdown-label")
      .toggleClass("ui-dropdown-label-required", value);
    self.noValid(!value);
    isRequired = value;
    ...
    for (var i = 0, length = arr.length; i < length; i++) {
      ...
      if (item.length)
        ...
        builder.push(
          template.format(
            item[propValue],
            value === item[propValue] ? ' selected="selected"' : "",
            item[propText]
          )
        );
    }
  ...
  self.make = function () {
    ...
    (self.attr("data-options") || "").split(";").forEach(function (item) {
      ...
      options.push(
        '<option value="{0}">{1}</option>'.format(
          item[1] === undefined ? item[0] : item[1],
          item[0]
        )
      );
    });
    ...
    var label = self.html();
    var html =
      '<div class="ui-dropdown"><span class="fa fa-sort"></span><select data-jc-bind="">{0}</select></div>'.format(
        options.join("")
      );
    var builder = [];
    ...
    if (label.length) {
      ...
      builder.push(
        '<div class="ui-dropdown-label{0}">{1}{2}:</div>'.format(
          isRequired ? " ui-dropdown-label-required" : "",
          icon ? '<span class="fa {0}"></span> '.format(icon) : "",
          label
        )
      );
      builder.push('<div class="ui-dropdown-values">{0}</div>'.format(html));
      self.html(builder.join(""));
    } else self.html(html).addClass("ui-dropdown-values");
...
COMPONENT("textbox", function () {
  ...
  self.validate = function (value) {
    if (input.prop("disabled") || !isRequired) return true;
    ...
    return validation
      ? self.evaluate(value, validation, true)
        ? true
        : false
      : value.length > 0;
  };
  ...
  self.required = function (value) {
    self
      .find(".ui-textbox-label")
      .toggleClass("ui-textbox-label-required", value);
    self.noValid(!value);
    isRequired = value;
    !value && self.state(1, 1);
  };
  ...
  self.make = function () {
    ...
    if (!icon2 && self.type === "date") icon2 = "fa-calendar";
      ...
      self.getter2 = function (value) {
        ...
        self
          .find(".ui-textbox-control-icon")
          .toggleClass("fa-times", value ? true : false)
          .toggleClass("fa-search", value ? false : true);
      };
    }
    ...
    increment &&
      !icon2 &&
      builder.push(
        '<div><span class="fa fa-caret-up"></span><span class="fa fa-caret-down"></span></div>'
      );
    increment &&
      self.event("click", ".fa-caret-up,.fa-caret-down", function (e) {
        var el = $(this);
        var inc = -1;
        if (el.hasClass("fa-caret-up")) inc = 1;
        self.change(true);
        self.inc(inc);
      });
    ...
    builder = [];
    builder.push(
      '<div class="ui-textbox-label{0}">'.format(
        isRequired ? " ui-textbox-label-required" : ""
      )
    );
...
COMPONENT("textarea", function () {
  ...
  self.required = function (value) {
    self
      .find(".ui-textarea-label")
      .toggleClass("ui-textarea-label-required", value);
    self.noValid(!value);
    isRequired = value;
    !value && self.state(1, 1);
  };
  ...
  self.make = function () {
    ...
    builder = [];
    builder.push(
      '<div class="ui-textarea-label{0}">'.format(
        isRequired ? " ui-textarea-label-required" : ""
      )
    );
    icon && builder.push('<span class="fa {0}"></span>'.format(icon));
    builder.push(content);
...
COMPONENT("template", function () {
  ...
  self.setter = function (value) {
    if (NOTMODIFIED(self.id, value)) return;
    if (!value) return self.classes("hidden");
    KEYPRESS(
      function () {
...
COMPONENT("error", function () {
  ...
  self.setter = function (value) {
    ...
    for (var i = 0, length = value.length; i < length; i++)
      builder.push(
        '<li><span class="fa fa-times-circle"></span> ' +
          value[i].error +
          "</li>"
      );
...
COMPONENT("grid", function () {
  ...
  self.click = function (index, row, button) {
  ...
  self.make = function (template) {
    ...
    self.html(
      '<div><div class="ui-grid-page"></div>{4}</div><div data-jc="pagination" data-jc-path="{0}" data-max="8" data-pages="{1}" data-items="{2}" data-target-path="{3}"></div>'.format(
        self.path,
        self.attr("data-pages"),
        self.attr("data-items"),
        self.attr("data-pagination-path"),
        table
          ? '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tbody></tbody></table>'
          : '<div class="ui-grid-body"></div>'
      )
    );
    self.event("click", "button", function () {
      switch (this.name) {
    ...
    setTimeout(function () {
      ...
      if (max === "auto")
        self.max = Math.floor(
          ($(window).height() - (self.element.offset().top + 110)) / height
        );
      else self.max = parseInt(max);
      if (self.max < 10) self.max = 10;
    }, 10);
  ...
  self.setter = function (value) {
    ...
    if (!output.length) {
      ...
      if (empty) {
        ...
        output.push(
          table
            ? '<tr><td style="text-align:center;padding:50px 0;background-color:white"><div style="padding:40px 20px;border:2px solid #F0F0F0;max-width:500px;margin:0 auto;border-radius:4px">{0}</div></td></tr>'.format(
                empty
              )
            : '<div class="empty"><i class="fa fa-database"></i>{0}</div>'.format(
                empty
              )
        );
      } else page.empty();
      ...
      if (format) page.html(format.replace(/\#/g, value.page));
      else page.empty();
    }
...
COMPONENT("form", function () {
  ...
  if (!MAN.$$form) {
    ...
    $(document).on("click", ".ui-form-container", function (e) {
      ...
      if (
        !(
          el.hasClass("ui-form-container-padding") ||
          el.hasClass("ui-form-container")
        )
      )
        return;
  ...
  self.resize = function () {
    ...
    var r = wh / 2 - fh / 2;
    if (r > 30) ui.css({ marginTop: r - 15 + "px" });
    else ui.css({ marginTop: "20px" });
  };
  ...
  self.make = function () {
    ...
    $(document.body).append(
      '<div id="{0}" class="hidden ui-form-container"><div class="ui-form-container-padding"><div class="ui-form" style="max-width:{1}"><div class="ui-form-title"><span class="fa fa-times ui-form-button-close" data-path="{2}"></span>{3}</div>{4}</div></div>'.format(
        self._id,
        width,
        self.path,
    ...
    enter === "true" &&
      self.event("keydown", "input", function (e) {
        e.keyCode === 13 &&
          !self.find('button[name="submit"]').get(0).disabled &&
          self.submit(hide);
      });
  };
  ...
  self.setter = function () {
    setTimeout2(
      ...
      function () {
        $("html").toggleClass(
          "noscroll",
          $(".ui-form-container").not(".hidden").length ? true : false
        );
      },
      50
    );
...
COMPONENT("repeater-group", function () {
  ...
  self.setter = function (value) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      if (!groups[name]) groups[name] = [value[i]];
      else groups[name].push(value[i]);
    }
...
COMPONENT("checkboxlist", function () {
  var self = this;
  var template = Tangular.compile(
    '<div class="{0} ui-checkboxlist-checkbox"><div><label><input type="checkbox" value="{{ id }}"><span>{{ name }}</span></label></div></div>'.format(
      self.attr("data-class")
    )
  );
  self.make = function () {
    ...
    self.make = function () {
      ...
      for (var i = 0, length = arr.length; i < length; i++) {
        ...
        datasource.push({
          id: item[1] === undefined ? item[0] : item[1],
          name: item[0],
        });
      }
    ...
    self.redraw = function (arr) {
      ...
      for (var i = 0, length = arr.length; i < length; i++) {
        ...
        if (typeof item === "string")
          ...
          builder.push(
            template({
              id: item[kv] === undefined ? item[kn] : item[kv],
              name: item[kn],
            })
          );
      }
      ...
      self.attr("data-button") &&
        builder.push(
          '<div class="col-md-12"><div class="ui-checkboxlist-selectall"><a href="javascript:void(0)"><i class="fa fa-toggle-on mr5"></i>{0}</a></div></div>'.format(
            self.attr("data-button")
          )
        );
      self.html(builder.join(""));
...
COMPONENT("dropdowncheckbox", function () {
  ...
  if (!window.$dropdowncheckboxtemplate)
    window.$dropdowncheckboxtemplate = Tangular.compile(
      '<div><label><input type="checkbox" value="{{ index }}" /><span>{{ text }}</span></label></div>'
    );
  ...
  self.make = function () {
    ...
    for (var i = 0, length = arr.length; i < length; i++) {
      ...
      if (self.type === "number") value = parseInt(value);
    ...
    var html =
      '<div class="ui-dropdowncheckbox"><span class="fa fa-sort"></span><div class="ui-dropdowncheckbox-selected"></div></div><div class="ui-dropdowncheckbox-values hidden">' +
      options.join("") +
      "</div>";
    ...
    if (content.length > 0) {
      element.empty();
      element.append(
        '<div class="ui-dropdowncheckbox-label' +
          (required ? " ui-dropdowncheckbox-label-required" : "") +
          '">' +
    ...
    self.event("click", ".ui-dropdowncheckbox", function (e) {
      var el = $(this);
      if (el.hasClass("ui-disabled")) return;
  ...
  function prepare(path, value) {
    if (NOTMODIFIED(path, value)) return;
    ...
    if (builder) container.removeClass(clsempty).empty().append(builder);
    else container.addClass(clsempty).empty().html(self.attr("data-empty"));
  ...
  self.setter = function (value) {
    ...
    if (value && value.length) {
      ...
      if (refresh) MAN.set(self.path, value);
    }

    container.find("input").each(function () {
      var index = parseInt(this.value);
      var checked = false;
      if (!value || !value.length) checked = false;
      else if (data[index]) checked = data[index];
      if (checked) checked = value.indexOf(checked.value) !== -1;
  ...
  self.state = function (type) {
    self
      .find(".ui-dropdowncheckbox")
      .toggleClass("ui-dropdowncheckbox-invalid", self.isInvalid());
  };

  if (window.$dropdowncheckboxevent) return;

  window.$dropdowncheckboxevent = true;
  $(document).on("click", function (e) {
    if (!window.$dropdowncheckboxelement) return;
    window.$dropdowncheckboxelement.addClass("hidden");
    window.$dropdowncheckboxelement = null;
  });
});
...
COMPONENT("codemirror", function () {
  ...
  self.make = function () {
    ...
    var content = self.html();
    self.html(
      '<div class="ui-codemirror-label' +
        (required ? " ui-codemirror-label-required" : "") +
        '">' +
        (icon ? '<span class="fa ' + icon + '"></span> ' : "") +
        content +
        ':</div><div class="ui-codemirror"></div>'
    );

    var container = self.find(".ui-codemirror");
    editor = CodeMirror(container.get(0), {
      lineNumbers: self.attr("data-linenumbers") === "true",
      mode: self.attr("data-type") || "htmlmixed",
      indentUnit: 4,
    });
  ...
  self.state = function () {
    self.element
      .find(".ui-codemirror")
      .toggleClass("ui-codemirror-invalid", self.isInvalid());
  };
});
...
COMPONENT("calendar", function () {
    ...
    for (var i = 0; i < days + from; i++) {
      ...
      if (i >= from) {
        ...
        if (!obj.isFuture && year === ty) {
          if (tm < month) obj.isFuture = true;
          else if (tm === month) obj.isFuture = td < index;
        }
      } else {
        indexEmpty++;
  ...
  self.toggle = function (el, value, callback, offset) {
    if (self.element.hasClass("hidden")) self.show(el, value, callback, offset);
    else self.hide();
    return self;
  };
  ...
  self.show = function (el, value, callback, offset) {
    ...
    self
      .css({ left: off.left + (offset || 0), top: off.top + h + 12 })
      .removeClass("hidden");
    self.click = callback;
    self.date(value);
    visible = true;
  ...
  self.date = function (value) {
    ...
    for (var i = 0; i < 42; i++) {
      ...
      if (item.isEmpty) cls.push("ui-calendar-disabled");
      else cls.push("ui-calendar-day");

      !empty && item.isSelected && cls.push("ui-calendar-selected");
      item.isToday && cls.push("ui-calendar-day-today");
      builder.push(
        '<td class="{0}" data-date="{1}-{2}-{3}">{3}</td>'.format(
          cls.join(" "),
          output.year,
          output.month,
          item.number
    ...
    self.html(
      '<div class="ui-calendar-header"><button class="ui-calendar-header-prev" name="prev" data-date="{0}-{1}"><span class="fa fa-chevron-left"></span></button><div class="ui-calendar-header-info">{2} {3}</div><button class="ui-calendar-header-next" name="next" data-date="{0}-{1}"><span class="fa fa-chevron-right"></span></button></div><table cellpadding="0" cellspacing="0" border="0"><thead>{4}</thead><tbody>{5}</tbody></table>'.format(
        output.year,
        output.month,
        self.months[value.getMonth()],
        value.getFullYear(),
        header.join(""),
        builder.join("")
      ) +
        (self.today
          ? '<div><a href="javascript:void(0)" class="ui-calendar-today">' +
            self.today +
            "</a></div>"
          : "")
    );
...
COMPONENT("disable", function () {
  ...
  if (validate) validate = validate.split(",").trim();
  ...
  self.setter = function (value) {
    ...
    self.find(selector).each(function () {
      ...
      } else el.toggleClass("ui-disabled", is);
    });

    validate &&
      validate.forEach(function (key) {
        jC.reset(key);
      });
  };
...
COMPONENT("confirm", function () {
  ...
  self.confirm = function (message, buttons, fn) {
    ...
    self.content(
      "ui-confirm-warning",
      '<div class="ui-confirm-message">{0}</div>{1}'.format(
        message.replace(/\n/g, "<br />"),
        builder.join("")
      )
    );
...
COMPONENT("pagination", function () {
  ...
  self.template = Tangular.compile(
    '<a href="#page{{ page }}" class="page{{ if selected }} selected{{ fi }}" data-page="{{ page }}">{{ page }}</a>'
  );
  self.readonly();
  self.make = function () {
    ...
    self.event("click", "a", function (e) {
      ...
      e.stopPropagation();
      var el = $(this);
      if (self.onPage) self.onPage(el.attr("data-page").parseInt(), el);
    });
  };
  ...
  self.getPagination = function (page, pages, max, fn) {
    ...
    if (page < half + 1) {
      pageTo++;
      if (pageTo > pages) pageTo--;
    }

    for (var i = pageFrom; i < pageTo + 1; i++) fn(i);
  };

  self.getPages = function (length, max) {
    var pages = (length - 1) / max;
    if (pages % max !== 0) pages = Math.floor(pages) + 1;
    if (pages === 0) pages = 1;
    return pages;
  };
  ...
  self.setter = function (value) {
    ...
    if (cachePages > 2) {
      ...
      if (prev <= 0) prev = cachePages;
      builder.push(
        '<a href="#prev" class="page" data-page="{0}"><span class="fa fa-arrow-left"></span></a>'.format(
          prev
        )
      );
    }
    ...
    self.getPagination(value.page, cachePages, max, function (index) {
      builder.push(
        self.template({ page: index, selected: value.page === index })
      );
    });

    if (cachePages > 2) {
      var next = value.page + 1;
      if (next > cachePages) next = 1;
      builder.push(
        '<a href="#next" class="page" data-page="{0}"><span class="fa fa-arrow-right"></span></a>'.format(
          next
        )
      );
    }
    ...
    if (cachePages > 1) {
      ...
      pluralize_pages.push.apply(
        pluralize_pages,
        self.attr("data-pages").split(",").trim()
      );
      pluralize_items.push.apply(
        pluralize_items,
        self.attr("data-items").split(",").trim()
      );
      info
        .empty()
        .append(
          Tangular.helpers.pluralize.apply(value, pluralize_pages) +
            " / " +
            Tangular.helpers.pluralize.apply(value, pluralize_items)
        );
    }
...
jC.formatter(function (path, value, type) {
  if (type === "date") {
    if (value instanceof Date) return value.format(this.attr("data-jc-format"));
...
COMPONENT("tagger", function () {
  ...
  self.setter = function (value) {
    ...
    elements.each(function () {
      ...
      if (format) {
        key = "format";
        if (cache[key]) format = cache[key];
        else format = cache[key] = self.arrow(format);
      }
      ...
      switch (type) {
        case "date":
          if (typeval === "string") val = val.parseDate();
          else if (typeval === "number") val = new Date(val);
          else val = "";
          break;

        case "number":
        case "currency":
          if (typeval === "string") val = val.parseFloat();
          if (typeof val !== "number") val = "";
          break;
      }

      if ((val || val === 0) && format) val = format(val);

      if (visible) {
        key = "visible";
        if (cache[key]) visible = cache[key];
        else visible = cache[key] = self.arrow(visible);
        var is = visible(val);
      ...
      if (val && !format) val = Ta.helpers.encode(val);
      ...
      if (this.innerHTML !== cache.def) this.innerHTML = cache.def;
    });
...
COMPONENT("photoupload", function () {
  ...
  self.make = function () {
    ...
    self.html('<img src="/img/avatar.svg" alt="" class="img-responsive" />');
    $(document.body).append(
      '<input type="file" id="{0}" class="hidden" accept="image/*" />'.format(
        id
      )
    );
    ...
    input.on("change", function (evt) {
      ...
      UPLOAD(self.attr("data-url"), data, function (response, err) {
        ...
        if (err) {
          ...
          else alert(self.attr("data-upload-error") || err.toString());
          return;
        }
